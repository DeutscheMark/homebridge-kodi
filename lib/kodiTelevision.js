        ...
        case "Controls":
            this.televisionService.getCharacteristic(Characteristic.Active)
                ...
                .on('set', (active, callback) => {
                    kodi.getStatus(this.config, this.log, (error, status) => {
                        this.log("Setting " + this.name + ": " + (!error && status));
                        setTimeout(() => {
                            this.televisionService.getCharacteristic(Characteristic.Active).updateValue((!error && status) ? Characteristic.Active.ACTIVE : Characteristic.Active.INACTIVE);
                        }, 100);
                    });
                    callback();
                });
            ...
            this.televisionService.getCharacteristic(Characteristic.ActiveIdentifier)
                .on('get', (callback) => {
                    this.log("Getting " + this.name + " Active Identifier: 1 (Home)");
                    callback(null, 1);
                })
                            ...
                            switch (activeIdentifier) {
                                ...
                                case 4:
                                    kodi.getActionResult(this.config, this.log, "GUI.ActivateWindow", { "window": "videos", "parameters": ["TVShowTitles"] }, (error, result) => {
                                        this.log("Setting " + this.name + " Active Identifier: " + activeIdentifier + " (TV shows)");
                                    });
                                    break;
                                case 5:
                                ...
                                case 7:
                                    kodi.getActionResult(this.config, this.log, "GUI.ActivateWindow", { "window": "videos", "parameters": ["MusicVideoTitles"] }, (error, result) => {
                                        this.log("Setting " + this.name + " Active Identifier: " + activeIdentifier + " (Music videos)");
                                    });
                                    break;
                                case 8:
                                    ...
                                    kodi.playerGetItem(this.config, this.log, playerid, [], (error, itemresult) => {
                                        if (!error && itemresult && itemresult.item) {
                                            ...
                                            if (itemtype == 'channel') {
                                                ...
                                                this.log("Getting " + this.name + " Active Identifier: " + activeIdentifier + " (" + inputName + ")");
                                                if (activeIdentifier) {
